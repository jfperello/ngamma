...
void GWMEventBuilder::Convert2SlowAnalyzedRoot() {
	...
	for(int i=m_rmin; i<=m_rmax; i++) {
		...
		system(unpack_command.c_str());
		converter.Convert2SlowAnalyzedRoot(sortfile, m_mapfile, m_SlowWindow, m_cutList);
		system(wipe_command.c_str());
...
void GWMEventBuilder::Convert2FastAnalyzedRoot() {
	...
	for(int i=m_rmin; i<=m_rmax; i++) {
		...
		system(unpack_command.c_str());
		converter.Convert2FastAnalyzedRoot(sortfile, m_mapfile, m_SlowWindow, m_FastWindowSABRE, m_FastWindowIonCh, m_cutList);
		system(wipe_command.c_str());
...
bool GWMEventBuilder::SetKinematicParameters() {
	return true;
}
